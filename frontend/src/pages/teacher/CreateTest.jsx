...
const CreateTest = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const { data } = await axios.get("http://localhost:5000/api/courses/teacher/dashboard", {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const testRes = await axios.post("http://localhost:5000/api/tests", testData, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
      const newTest = testRes.data;

      await axios.put(`http://localhost:5000/api/tests/${newTest._id}/questions`, { questions }, {
        headers: { Authorization: `Bearer ${user.token}` }
      });
